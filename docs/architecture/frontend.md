...
| `ProfileManager` | `profiles/` | Parental controls: pause, bedtime, content filters, scheduling. Split into sub-components. |
| `SettingsView` | `SettingsView.tsx` → `settings/` | Barrel re-export; individual components split into `components/settings/` |
