...
| File | Role |
|----|----|
| `backend/core/dry_run.py` | `is_dry_run()` and `block_unmocked_mutation()` |
| `backend/features/*/service.py` | Dry-run guards for mutations that call the eero client directly |
| `backend/core/facade.py` | Dry-run guards for facade-routed mutations (blacklist, port forwards, reservations) |
| `backend/tests/test_dry_run.py` | Unit tests for flag parsing, blocking, and service integration |
